		...
		private void PersonQuery_Load(object sender, System.EventArgs e)
		{
			...
			for (int x=0;x<ds.Tables["bookid"].Rows.Count;x++)
			{
				string sql2="select book.BID as 图书编号,BName as 图书名,BWriter as 作者,BPublish as 出版社,BDate as 出版日期,BPrice as 价格,"+
                    ...
                    "dateAdd(m,(select longTime from identityinfo where identityname=(select identityname from person where PID='" + this.Tag.ToString().Trim() + "')),ODate) as 应还日期 from book,bookOut where book.BID = bookOut.BID and book.BID = '" + ds.Tables["bookid"].Rows[x][0] + "'" +
