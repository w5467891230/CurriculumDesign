...
namespace LibraryMIS
{
	...
	public class BookOut : System.Windows.Forms.Form
	{
		...
		private void textPID_KeyDown(object sender, System.Windows.Forms.KeyEventArgs e)
		{
			...
			{
				oleConnection1.Open();
				string sql1 = "select PName as 姓名,PSex as 性别,PN as 身份证,PMoney as 罚款,identityname as 身份 "+
				...
				for (int x=0;x<ds.Tables["bookid"].Rows.Count;x++)
				{
                    ...
                    string sql22 = "select book.BID as 图书编号,BName as 图书名,BWriter as 作者,BPublish as 出版社,BDate as 出版日期,BPrice as 价格," +
                    "[type] as 类型,ODate as 借书日期," +
                    "dateAdd(m,(select longTime from identityinfo where identityname=(select identityname from person where PID='" + this.textPID.Text.Trim() + "')),ODate) as 应还日期 from book,bookOut where book.BID = bookOut.BID and book.BID = '" + ds.Tables["bookid"].Rows[x][0] + "'" +
                    " and PID='" + textPID.Text.Trim() + "'";
